...
def main(page: Page):
    ...
    page.route = '/'                 # defino a raiz como a rota que desejo ir

    rotas = ['/', '1', '2', '3', '4']          # lista das rotas do sistema
    ...
    class Botoes():                    # classe para criar dois botões: um em foco e outro sem foco
        ...
        def botoes_navegacao():
            botoes_row = Row(
                [
                    Botoes().criar(rotas[i], navegar, True) if page.route == rotas[i] or page.route == f'/{rotas[i]}'  else Botoes().criar(rotas[i], navegar, False)
    ...
    def new_route(route):              # função que define qual será os comandos executados no evento "page.on_route_change" chamado pelo "page.update"
        ...
        elif page.route == rotas[1] or page.route == f'/{rotas[1]}':
            # a partir daqui eu construo minha página
            page.add(Text('Esta é a primeira página', size=30))
            Botoes.botoes_navegacao()

        elif page.route == rotas[2] or page.route == f'/{rotas[2]}':
            # a partir daqui eu construo minha página
